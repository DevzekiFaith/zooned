...
export default function AIAssistant() {
  ...
  const [messages, setMessages] = useState([
    { from: "ai", text: "Hi! I’m your assistant. How can I help you navigate or use the app?" }
  ]);
      ...
      {open && (
        <div className="fixed bottom-20 right-6 z-50 w-80 bg-white rounded-xl shadow-xl flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-2" style={{ maxHeight: 300 }}>
            {messages.map((msg, i) => (
              <div key={i} className={msg.from === "ai" ? "text-left" : "text-right"}>
                <span className={msg.from === "ai" ? "bg-blue-100 text-blue-800" : "bg-green-100 text-green-800"} style={{ borderRadius: 8, padding: "4px 8px", display: "inline-block" }}>
                  {msg.text}
                </span>
              </div>
            ))}
          ...
          <form onSubmit={handleSend} className="flex border-t">
            ...
            />
            <button type="submit" className="px-4 text-blue-600 font-bold">Send</button>
          </form>
        </div>
      )}
    </>
